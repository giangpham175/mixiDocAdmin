...
const bloodstorage = {
    ...
    actions: {
        async addBlood({commit}, blood) {
            const docRef = await firestore().collection(collName).add(blood);
            blood.id = docRef.id;
        ...
        async updateBlood({commit}, payload) {
            await firestore().collection(collName).doc(payload.blood.id).set(payload.blood);
            commit('UPDATE_BLOOD', payload);
        },
        async removeBlood({commit}, blood) {
            await firestore().collection(collName).doc(blood.id).delete();
