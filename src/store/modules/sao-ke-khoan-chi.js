...
const expenses = {
    ...
    actions: {
        async addExpense({commit}, expense) {
            const docRef = await firestore().collection(collName).add(expense);
        ...
        async updateExpense({commit}, payload) {
            await firestore().collection(collName).doc(payload.expense.id).set(payload.expense);
            commit('UPDATE_EXPENSE', payload);
        },
        async removeExpense({commit}, expense) {
            await firestore().collection(collName).doc(expense.id).delete();
